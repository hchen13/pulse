...
以下是 **{repo_name}** 过去 {days} 天的 Issues 数据（{issue_count} 条），代表用户真实声音。
...
以下是 **{repo_name}** 过去 {days} 天的 Pull Requests 数据（{pr_count} 条）。
...
- 3-5 条 bullet，聚焦社区关注的问题方向和动机
- 有多人同时贡献的方向特别值得关注
- 关注：社区在修什么（官方还没修的）？社区在补充什么能力（路线图之外的）？
...
### Merged PRs（{merged_pr_count} 条）
（官方选择合并的 PR 是最直接的优先级声明）
{merged_prs_text}

### 非 main 分支 Commits（{branch_commit_count} 条）
（代表官方正在推进但尚未发布的工作方向）
{branch_commits_text}
...
- 3-5 条 bullet，聚焦投入方向和战略信号
- Merge 决策是最直接的优先级声明，重点关注官方自己发起的 merged PR
- 关注：官方在把资源往哪里押注？commit 集中在哪个模块或方向？
...
### Main 分支 Commits（{main_commit_count} 条）
（代表已经稳定、准备或已经发布的工作）
{main_commits_text}

### Releases（{release_count} 条）
{releases_text}
...
- 3-4 条 bullet，聚焦节奏和阶段判断
- 关注：发版频率如何？每次 release 的重量级？项目处于什么开发阶段（快速迭代 / 稳定维护 / 大版本冲刺）？
...
以下是多个 AI agent harness 项目在 {date} 的分析报告（每个项目包含四个维度）：

{repo_summaries}

---

请输出跨项目战略综合分析，中文，**直接用以下结构输出，不要加前言或标题前缀**：
...
跨项目的社区贡献方向：社区层面的共同信号是什么？社区集体在尝试推动什么？（3-4 条 bullet）
...
【输出规范】禁止 emoji，禁止图标字符，要解读和洞察，不要纯描述，不要重复各项目内容。
"""


class LLMAnalyzer:
    def __init__(self, config: AnalysisConfig, db_path: str):
        self.config = config
    ...
    def _analyze_issues(self, repo_name: str, data: Dict, days: int) -> Optional[str]:
        """维度 1：用户痛点与需求（Issues）"""
        issues = data["issues"]
    ...
    def analyze_global(self, repo_summaries: Dict[str, str]) -> Optional[str]:
        """生成跨 repo 的四角度综合分析"""
