...
# ── 维度 1：用户痛点与需求（Issues） ──────────────────────────────────────────────
ISSUES_PROMPT = """以下是 **{repo_name}** 过去 {days} 天的 Issues 数据（共 {issue_count} 条）：

{issues_text}

---

请分析上述 issues，第一行必须是"## 用户痛点与需求"，直接开始，不加前言。
"""


# ── 维度 2：社区在解决什么（PRs） ──────────────────────────────────────────────
PRS_PROMPT = """以下是 **{repo_name}** 过去 {days} 天的 Open Pull Requests 数据（共 {pr_count} 条）：

{prs_text}

---

请分析上述 PRs，第一行必须是"## 社区在解决什么"，直接开始，不加前言。
"""


# ── 维度 3：官方工作重心与方向（非 main 分支 commits + merged PRs） ─────────────
BRANCH_COMMITS_PROMPT = """以下是 **{repo_name}** 过去 {days} 天的官方工作信号：
...
请分析官方工作重心，第一行必须是"## 官方工作重心与方向"，直接开始，不加前言。
"""


# ── 维度 4：版本进度与节奏（main 分支 commits + releases） ────────────────────
MAIN_PROGRESS_PROMPT = """以下是 **{repo_name}** 过去 {days} 天的版本进度数据：
...
请分析版本进度与节奏，第一行必须是"## 版本进度与节奏"，直接开始，不加前言。
"""
...
class LLMAnalyzer:
    ...
    def _call_claude(self, prompt: str, analyst: str = "synthesis") -> Optional[str]:
        ...
        try:
            ...
            # 过滤 claude CLI 可能输出的 thinking/preamble 行（出现在第一个 ## 标题之前）
            import re as _re
            first_header = _re.search(r'^#{1,3} ', output, _re.MULTILINE)
            if first_header:
                output = output[first_header.start():]
            return output
...
请将上述四个维度整合成一份完整的项目分析报告，保留四个维度标题，中文输出。
"""
